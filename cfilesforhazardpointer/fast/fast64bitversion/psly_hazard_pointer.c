...
#define ARRAYNUMb ARRNUM
#define ARRAYNUM  (1 << ARRAYNUMb)
#define ARRAYPERb IDXNUM
		...
		if(tailIndex == queue->tail){
			...
			//	printf("loop here %d\n", count); 
				if(!__sync_bool_compare_and_swap(&queue->tail, tailIndex, (((tailIndex & TAILVERSIONBIT) + TAILVERSIONONE) & TAILVERSIONBIT)|(nextIndex & TAILIDXBIT)))
					...
					printf("true\n");
					printf("%d, %d    not occur~~~~\n", indexTail, nextIndex & TAILIDXBIT);
				}
			}	
		}
	}	
			...
			if((start.tv_usec & 111) >= 0) {
				...
				if(refc < ((1 << REFCB) - 1)) { 
					if( __sync_bool_compare_and_swap(refCount + key, refc, refc + 1)) {
						psly_record(key * (ARRAYLEN)/* * INDEXBITS*/);
...
int main(int argc, char** argv) {
	...
	for(int i = 0; i < numOfItems; ++i) { 
		...
		if(refCount[i] != psly_search(i * (ARRAYLEN))) 
        	++errCount;
	}
	//sleep(1);
	gettimeofday(&end, NULL);
	float record_use=(end.tv_sec-start.tv_sec)+(end.tv_usec-start.tv_usec) / 1000000.0;//微秒   
	gettimeofday(&start, NULL);	
	pthread_mutex_unlock(&mutex);
	for(int i = 0; i < num; ++i)
...
//	int errCount = 0;
	...
	int add = 0;
	for(int i = 0; i < numOfItems; ++i) {
//		printf("%d, %d\n", refCount[i], psly_search(i * (ARRAYLEN-1)));
		if(refCount[i] != psly_search(i * (ARRAYLEN))) {
			++errCount;
//			printf("%d refCount: %d, %d\n", i, refCount[i], k = psly_search(i * (ARRAYLEN)));	
	...
	printf("record_use: %f\n\n", record_use);	
	printf("remove_use: %f\n\n", time_use);
	Record* head = map.lists[0].head;
	Record* tail = map.lists[0].tail;
	while(head != tail) {
		printf("one\n");
		head = takeNext(head->nextRecord);
	}		
	return 0;	
}
