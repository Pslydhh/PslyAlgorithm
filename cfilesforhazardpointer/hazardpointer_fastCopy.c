...
void return_Record(Record* record) {
  ...
  record->next = local;
    ...
    for(;;) {
        ++count;
        if(count > 100000)
          printf("RETURN NNNNNNNNNNNNNNNNOn");
        int tailIndex = (queue->tail);
        int indexTail = tailIndex & PSLY_Record_TAILIDXBIT;
...
int psly_record(void* pointer) {
  long key = (long) pointer;
  RecordList* list = map.lists[key & (131072-1)];
...
int main(int argc, char** argv){
	int n = 0;
	INIT_RESOURCE(131072); for(int k = 0; ;++k){
	printf("%d times\n", k);
	float time_use=0;
