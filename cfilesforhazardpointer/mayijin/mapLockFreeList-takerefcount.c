...
#define UNINIT_RESOURCE(Record, listNum, num_, errQueues, num)  \
	int count = 0;\
    for(int i = 0; i < (1 << PSLY_##Record##_ARRNUM); ++i){ \
...
int main(int argc, char** argv) {
  ...
  printf("%d times\n", fl);
  if((fl % 3) == 1)
  	printf("%d threads extracts %d numbers randomly from 0 to 65535, records it, check it, and outputs the count of 0-%d:", num, recordremove, checknum -1);
  if((fl % 3) == 2)
	printf("%d threads extracts %d numbers sequencelly from 0 to 65535, removes it until zero, check it, and outputs the count of 0-%d (all 0):", num, checknum - 1);
  if((fl % 3) == 0)
	printf("%d Each thread first record(randomly)/then remove(sequencelly) %d numbers randomly from 0 to 65535, check it, and outputs the count of 0-%d (all 0):", num, recordremove, checknum - 1);
  printf("\n\n");
  pthread_mutex_lock(&mutex);
  gettimeofday(&start, NULL);
  pthread_setconcurrency(sysconf(_SC_NPROCESSORS_ONLN));
  for(int i = 0; i < num; ++i) {
    if(pthread_create(pids + i, NULL, recordremoveRoutine, (void*) i)) {
  ...
  printf("total_use: %f seconds, numOfItems: %d\n\n", time_use, numOfItems);
  int k = 0;
  for(int i = 0; i < LISTNUM; ++i) {
  ...
  UNINIT_RESOURCE(Record, LISTNUM, k, &errQueueNum, fl - exeTime);
