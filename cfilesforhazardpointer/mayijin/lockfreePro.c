...
int main(int argc, char** argv) {
  ...
  if((fl % 3) == 2)
	printf("%d threads remove the all numbers randomly from 0 to 65535, check it, and outputs the count of 0-%d (all 0):", num, checknum - 1);
  if((fl % 3) == 0)
	printf("%d Each thread first record/then remove %d numbers randomly from 0 to 65535, check it, and outputs the count of 0-%d (all 0):", num, recordremove, checknum - 1);
  printf("\n\n");
  fflush(stdout);
  pthread_mutex_lock(&mutex);
  gettimeofday(&start, NULL);
