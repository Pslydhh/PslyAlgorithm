...
//atomic<long> fast_push;
//atomic<long> slow_push;
//atomic<long> deq_fast_num;
//atomic<long> deq_slow_num;
...
class WaitFreeQueue {
  ...
  class Handle {
    friend class WaitFreeQueue;
    Handle(WaitFreeQueue<T>* queue_outer) {
      this->next = nullptr;
      //this->prev = nullptr;
      this->hzd_node_id = -1;
      this->Er.id = 0;
      ...
      this->cleanId = 0;
      {
        ...
        this->Dh = this->next.load(memory_order_seq_cst);
  ...
  class HandleHolder {
    friend class WaitFreeQueue;
    HandleHolder(WaitFreeQueue<T>* queue_outer_) : /*queue_outer(queue_outer_),*/ handle(new Handle(queue_outer_)) {
    }

    /*  for unlink thread's handle while thread over, so threads must over before this queue destructs.
    ~HandleHolder() {
      unique_lock<mutex> control(queue_outer->safe_for_reclaim);
  ...
  T* help_enq(Handle* th, Cell* c, long i) {
    ...
    unsigned long prev_pop_items = th->pop_items;
...
public:
  WaitFreeQueue<T>(int spin = 16, int push_fast = 32, int pop_fast = 16, int bytes = (1 << 15), int bytes_thread = (1 << 12)) : ei(1), di(1), hi(0), clean_id(0),
  ...
  ~WaitFreeQueue<T>() {
    ...
    */
    ...
    if (tail != nullptr) {
      Handle* first_tail = tail;
      do {
        Handle* tmp = tail->next;
        delete tail->spare;
        delete tail;
        tail = tmp;
      } while (tail != first_tail);
    }
  ...
  unique_ptr<T> pop() {
    ...
    if (th->cleanId > 0) {
      ...
      else if (th->pop_items >= MAX_CELLS) {
        th->pop_items = 0;
        th->cleanId = cleanup(th, 0);
      }
    }
